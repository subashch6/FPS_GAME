...
GLuint AbstractShader::loadShader(const char* vertShaderPath, const char* fragShaderPath)
{
	...
	if (params == GL_FALSE)
	{
		...
		//fprintf(stderr, "vertShaderIssue: %s", infoLog);std::cout
		exit(EXIT_FAILURE);
	}
...
GLint AbstractShader::uniformLocation(const char* string)
{
	return glGetUniformLocation(program, string);
}

void AbstractShader::loadFloat(GLint location,float fl)
{
